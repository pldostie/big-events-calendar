#!/usr/bin/env python3
"""
Big Events Calendar Scraper v2
================================
...
Rules:
  ...
  - World Cup = "Country A vs Country B"
...
def scrape_f1():
    ...
    for year in [YEAR, YEAR+1]:
        try:
            ...
            for s in r.json():
                try:
                    start=date.fromisoformat(s["date_start"][:10])
                    end=date.fromisoformat(s.get("date_end",s["date_start"])[:10])
                    events.append(Evt(
...
def wiki_players(slam):
    ...
    try:
        ...
        pats=[
            r"([A-Z][a-zA-Z'\-]+(?:\s[A-Z][a-zA-Z'\-]+)+)\s+(?:def\.|defeated|d\.)\s+([A-Z][a-zA-Z'\-]+(?:\s[A-Z][a-zA-Z'\-]+)+)",
            r"([A-Z][a-zA-Z'\-]+(?:\s[A-Z][a-zA-Z'\-]+)+)\s+vs\.?\s+([A-Z][a-zA-Z'\-]+(?:\s[A-Z][a-zA-Z'\-]+)+)",
        ]
        for block in soup.find_all(["p","li","td","th"]):
            raw=block.get_text(separator=" ",strip=True); low=raw.lower()
            for key,kws in [("QF",["quarterfinal"]),("SF",["semifinal"]),("Final",["final"])]:
                if any(k in low for k in kws):
                    for pat in pats:
                        for m in re.finditer(pat,raw):
                            p1,p2=m.group(1).strip(),m.group(2).strip()
                            if len(p1)>3 and len(p2)>3 and p1!=p2 and (p1,p2) not in res[key]:
...
def scrape_tennis():
    ...
    for slam in GRAND_SLAMS:
        ...
        sc=slam_sched(slam); pl=wiki_players(slam)
        ...
        for i,day in enumerate(sc["QF"]):
            ...
            pairs=pl.get("QF",[])
            if i<len(pairs):
                p1,p2=pairs[i]; n=f"{slam['name']} QF ({g}) -- {p1} vs {p2}"
            else:
                n=f"{slam['name']} QF ({g}) -- TBD"
            events.append(Evt(name=n,sport="Tennis",start=day,end=day,
                location=slam["location"],url=wurl,
                description="Quarterfinal -- check tournament site for exact time"))
        for rk,g,pi in [("SF-W","Women's",0),("SF-M","Men's",1),("F-W","Women's",0),("F-M","Men's",1)]:
            day=sc[rk][0]; lbl="SF" if rk.startswith("SF") else "Final"
            pairs=pl.get("SF" if lbl=="SF" else "Final",[])
            if pi<len(pairs):
                p1,p2=pairs[pi]; n=f"{slam['name']} {lbl} ({g}) -- {p1} vs {p2}"
            else:
                n=f"{slam['name']} {lbl} ({g}) -- TBD"
            events.append(Evt(name=n,sport="Tennis",start=day,end=day,
                location=slam["location"],url=wurl,
                description=f"{g} {lbl} -- check tournament site for exact time"))
...
def scrape_ufc():
    ...
    try:
        ...
        for row in table.find_all("tr")[1:]:
            ...
            # fighters go IN the title
            title=f"{ename} -- {main}" if main else ename
            desc="Main card: ~10:00 PM ET"
...
# ---------------------------------------------------------------------------
# NFL Bears -- regular season + playoffs, "Bears vs Opponent" in title
# ---------------------------------------------------------------------------
    ...
    for season in [YEAR,YEAR+1]:
        ...
        try:
            ...
            for row in tbody.find_all("tr"):
                ...
                except: is_po=any(k in str(week).lower() for k in ["wild","division","conference","super"])
                phase="Playoffs" if is_po else f"Regular Season Week {week}"
                desc=f"NFL {phase}"
                if gtime: desc+=f"\nKickoff: {gtime} ET"
                events.append(Evt(name=name,sport="NFL",start=d,end=d,location=place,url=url,description=desc))
            if events: break
        except Exception as e:
...
# ---------------------------------------------------------------------------
NBA_STARTS={2025:date(2025,10,22),2026:date(2026,10,21),2027:date(2027,10,21)}

def scrape_nba():
    events=[]
    for year,start in NBA_STARTS.items():
        if start>=TODAY-timedelta(30):
            events.append(Evt(name=f"NBA Season {year}-{year+1} Begins",sport="NBA",
...
# ---------------------------------------------------------------------------
# NHL Canadiens -- all games, "Canadiens vs Team" in title
# ---------------------------------------------------------------------------
def scrape_nhl_habs():
    events=[]
    for ey in [YEAR+1,YEAR]:
        ...
        try:
            ...
            for row in tbody.find_all("tr"):
                if "thead" in " ".join(row.get("class",[])): continue
                cells={td.get("data-stat",""):td.get_text(strip=True) for td in row.find_all(["td","th"])}
                ds=cells.get("date_game",""); oa=cells.get("opp",""); ha=cells.get("game_location","")
                ...
                events.append(Evt(name=name,sport="NHL",start=d,end=d,location=place,url=url,
                    description=f"NHL Regular Season {ey-1}-{ey}"))
            if events: break
        except Exception as e:
        ...
        try:
            ...
            for table in soup.find_all("table"):
                for row in table.find_all("tr"):
                    ...
                    en=next((c for c in cells if any(k in c.lower() for k in RL_KW) and len(c)<100),None)
                    if not en: continue
                    s=e=None
                    for c in cells:
                        s,e=pdrange(c,yr)
                        if s: break
                    if s: found.append(Evt(name=f"RLCS {slug} -- {en}",sport="Rocket League",
...
# ---------------------------------------------------------------------------
# FIFA World Cup 2026 -- "Country A vs Country B" always in title
# ---------------------------------------------------------------------------
def scrape_world_cup():
    events=[]; seen=set()
    pages=[
        ("https://en.wikipedia.org/wiki/2026_FIFA_World_Cup_group_stage","Group Stage"),
        ("https://en.wikipedia.org/wiki/2026_FIFA_World_Cup_knockout_stage","Knockout"),
        ("https://en.wikipedia.org/wiki/2026_FIFA_World_Cup","World Cup"),
    ]
    for url,phase in pages:
        try:
            soup=get(url,delay=2.0)
            for table in soup.find_all("table",class_=re.compile("wikitable")):
                ...
                for i,h in enumerate(hdrs):
                    if h in {"date","match date"} and "date" not in col: col["date"]=i
                    elif h in {"time","kickoff","kick-off"} and "time" not in col: col["time"]=i
                    elif h in {"team1","home team","team 1","team a"} and "t1" not in col: col["t1"]=i
                    elif h in {"team2","away team","team 2","team b"} and "t2" not in col: col["t2"]=i
                    elif h in {"venue","stadium","ground"} and "venue" not in col: col["venue"]=i
                    elif ("group" in h or "round" in h) and "round" not in col: col["round"]=i
                ...
                for row in rows[1:]:
                    ...
                    def ct(k):
                        i=col.get(k)
                        return cells[i].get_text(separator=" ",strip=True) if i is not None and i<len(cells) else ""
                    ds=ct("date"); ts=ct("time"); t1=ct("t1").strip(); t2=ct("t2").strip()
                    venue=ct("venue").strip(); rnd=ct("round").strip() or phase
                    if not t1 and not t2: continue
                    d=pdate(ds,2026)
                    if not d: continue
                    # Title: always team names, never just city
                    if t1 and t2: title=f"{t1} vs {t2}"
                    elif t1: title=f"{t1} -- {rnd}"
                    else: title=f"{rnd} Match"
                    key=(title.lower(),d)
                    if key in seen: continue
                    seen.add(key)
                    desc_parts=[]
                    if rnd and rnd!=phase: desc_parts.append(f"Round: {rnd}")
                    if ts: desc_parts.append(f"Kickoff: {ts}")
                    if venue: desc_parts.append(f"Venue: {venue}")
                    events.append(Evt(name=f"FIFA World Cup -- {title}",sport="World Cup",
                        start=d,end=d,location=venue,url=url,description="\n".join(desc_parts)))
            if events: break
        except Exception as e:
            print(f"  [WC] {url}: {e}",file=sys.stderr)
        ...
        for table in soup.find_all("table",class_=re.compile("wikitable")):
            ...
            for row in rows[1:]:
                ...
                def ct(k):
                    i=col.get(k)
                    return cells[i].get_text(separator=" ",strip=True) if i is not None and i<len(cells) else ""
                title=ct("title"); dev=ct("dev"); pub=ct("pub"); plat=ct("plat"); dr=ct("date")
                rel=pdate(dr,year)
                if not rel or not title: continue
                games.append({"title":title,"dev":dev,"pub":pub,"plat":plat,"rel":rel,"url":url})
    except Exception as e:
...
def scrape_ubisoft_games():
    ...
    # Fallback: Ubisoft website (JS-rendered, best-effort)
    if not any("ubisoft" in g["pub"].lower() or "ubisoft" in g["dev"].lower() for g in all_g):
        try:
            soup=get("https://www.ubisoft.com/en-us/games/upcoming",delay=2.0)
            for tag in soup.find_all(["h2","h3","h4"]):
                text=tag.get_text(strip=True); d=pdate(text,YEAR)
                if d and 3<len(text)<80:
                    all_g.append({"title":text,"dev":"Ubisoft","pub":"Ubisoft","plat":"","rel":d,
                        "url":"https://www.ubisoft.com/en-us/games/upcoming"})
        except Exception as e:
            print(f"  [Ubisoft] website: {e}",file=sys.stderr)
    seen=set()
    for g in all_g:
        if "ubisoft" not in g["pub"].lower() and "ubisoft" not in g["dev"].lower(): continue
        k=g["title"].lower()
...
def scrape_aaa_games():
    ...
    for g in all_g:
        pl=g["plat"].lower()
        is_aaa=any(p in g["pub"].lower() or p in g["dev"].lower() for p in AAA_PUBLISHERS)
        is_multi=("pc" in pl or "windows" in pl) and any(p in pl for p in ["playstation","xbox","ps5","ps4","series"])
...
def build_ics(events):
    ...
    cal.add("x-wr-caldesc",
        "F1 - Tennis Grand Slams - UFC - NFL Bears - NBA - NHL Canadiens"
        " - Rocket League - World Cup - Ubisoft - AAA Games")
